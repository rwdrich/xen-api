...
let policy_pv         = Map_check.(field Xapi_globs.cpu_info_policy_pv_key string)
let policy_hvm        = Map_check.(field Xapi_globs.cpu_info_policy_hvm_key string)
...
let get_flags_for_vm ~__context vm cpu_info =
  let policy_key =
    ...
    | `hvm | `pv_in_pvh ->  policy_hvm
    | `pv -> policy_pv
  in
  let vendor = List.assoc cpu_info_vendor_key cpu_info in
  let policy = Map_check.getf policy_key cpu_info in
  (vendor, policy)
